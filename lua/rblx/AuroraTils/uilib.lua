local Configs_HUB = {
  ...
  function MakeNotifi(Configs)
    local Title = Configs.Title or "Aurora Hub"
  ...
  function MakeWindow(Configs)
    ...
    if KeySystem then
      ...
      ConfirmButton.MouseButton1Click:Connect(function()
        if type(KeyKey) == "string" then
          if TextBox.Text == KeyKey then
              KeyVerify = true
          end
        elseif type(KeyKey) == "table" then
...
-- there is no mailstealer at the end :)
-- some keywords to get you down here xD
-- discord.gg loadstring http https mailstealer